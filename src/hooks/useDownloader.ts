...
// TODO: UX - Add network diagnostics and suggestions for firewall/NAT issues
// TODO: UX - Add retry mechanism with exponential backoff
// TODO: Security - Consider password hashing (currently sent plaintext over encrypted WebRTC)
...
const getZipFilename = (): string => `bodega-beam-download-${Date.now()}.zip`
...
export function useDownloader(uploaderPeerID: string): {
  ...
  const stopDownload = useCallback(() => {
    // TODO(@ali): Continue here with stop / pause logic
    if (dataConnection) {
